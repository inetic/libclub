...
void hub::fuse(Socket&& xsocket, const OnFused& on_fused)
{
    ...
    static const size_t buffer_size = sizeof(NET_PROTOCOL_VERSION)
                                    + sizeof(_id);
    ...
    async_exchange(*socket, buffer(*tx_bytes), 10000,
        ...
        (error_code error, Bytes&& rx_bytes) {
            ...
            binary::decoder d( reinterpret_cast<uint8_t*>(rx_bytes.data())
                             , rx_bytes.size());
...
void hub::node_received_unreliable_broadcast(const Bytes& bytes)
{
    ...
    binary::decoder d( reinterpret_cast<const uint8_t*>(bytes.data())
                     , bytes.size());
